...
run('INSERT INTO rounds (id, name, "order", answerMode, pointsPerQuestion, timeLimitSeconds, gapEnabled, gapSeconds, instructions) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [r1, 'Verify', 1, 'MCQ', 10, 30, 0, 0, 't']);
run('INSERT INTO questions (id, roundId, "order", text, mediaType, mediaUrl, options, correctOptionKey, pointsOverride, timeLimitOverrideSeconds, gapEnabledOverride, gapSecondsOverride) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [q1, r1, 1, 'Q?', 'none', null, JSON.stringify([{ key: 'A', text: 'w' }, { key: 'B', text: 'r' }, { key: 'C', text: 'x' }, { key: 'D', text: 'y' }]), 'B', null, null, null, null]);
...
async function runTests() {
  ...
  assert('startQuiz -> QUESTION_SHOWN', startState.phase === 'QUESTION_SHOWN');
  await firstTickP; // ensures at least one timer tick flowed before we proceed
  await sleep(1200);

  const lockAck = await emitP(c1Client, 'candidate:lockAnswer', { candidateId: c1, optionKey: 'A' });
  assert('c1 lockAnswer success', lockAck && lockAck.success === true);
  await sleep(1200);
  ...
  assert('c2 does NOT see c1.optionKey (redacted)', c1Lock && c1Lock.optionKey === undefined);
